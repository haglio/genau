...
' that says Genau VR instead, and this picks it up; the run makes it for the run
' after, so a checkout that has never started launches exactly as it used to.
namedPython = scriptDir & "\.venv\Scripts\GenauVR-GenauVR.exe"
If fso.FileExists(namedPython) Then venvPython = namedPython

If fso.FileExists(venvPython) Then
  cmd = """" & venvPython & """ -m genau_vr"
Else
  cmd = "pythonw -m genau_vr"
End If
