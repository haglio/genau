...
class TestRecord:
    def test_writes_the_answer_into_the_clip_sidecar(self, tmp_path):
        ...
        clip = lib / "w" / "Emy Reyes - Angels of Debauchery 8.mp4"
        clip.parent.mkdir(parents=True)
        clip.write_bytes(b"x")
        sidecar = meta / "w" / "Emy Reyes - Angels of Debauchery 8.json"
        sidecar.parent.mkdir(parents=True)
        sidecar.write_text(json.dumps({
            "version": {"group": "Emy Reyes - Angels of Debauchery 8", "processed": False},
            "video": {"action": "Alpha"},
...
class TestMatchLibrary:
    def _library(self, tmp_path):
        ...
        for rel, size, payload in (
            ...
            ("w/Emy Reyes - Angels of Debauchery 8.mp4", 50,
             {"clip": {"compilation": "Vol1", "index": 9, "performer": "Emy Reyes"}}),
    ...
    def test_records_the_scene_each_clip_was_cut_from(self, tmp_path):
        ...
        cut_from_it = lib / "w" / "Emy Reyes - Angels of Debauchery 8.mp4"
        scene = lib / "other" / "Emy-Reyes_540-izB4YKFa.mp4"
