...
class TestReadClip:
    def test_reads_clip_field(self, tmp_path):
        ...
        v = _clip(lib, meta, "w/Ann Bly - POV Scene 2.mp4", "Vol6", 9,
                  "POV Scene 2", "Ann Bly")
        c = read_clip(v, meta)
        assert c["compilation"] == "Vol6"
        assert c["index"] == 9
        assert c["performer"] == "Ann Bly"
...
class TestClipNav:
    ...
    def test_clip_of_ignores_a_performer_with_several_scenes(self, tmp_path):
        """A title like "Jane Doe To the Brink" must not claim every Jane Doe
        ...
        lib, meta = tmp_path / "videos" / "videos", tmp_path / "videos" / "metadata"
        clip = _clip(lib, meta, "w/Jane Doe - Jane Doe To the Brink.mp4", "Vol1", 11,
