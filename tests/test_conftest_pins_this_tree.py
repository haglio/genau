...
another's tests imports the wrong `genau`.  Loud when the two have drifted
...
def test_the_packages_under_test_are_this_trees():
    """The cheap half: whatever the path ended up as, the package this suite
    is about came out of the tree the suite lives in."""
    assert Path(genau.__file__).resolve().parent.parent == PROJECT_ROOT, (
        f"genau came from {genau.__file__}, not {PROJECT_ROOT}"
    )


def _decoy_tree(tmp_path: Path) -> Path:
    """A directory carrying this repo's package name and nothing real.

    Its `genau.window` has none of the real one's names, so a decoy that wins
    cannot pass by accident.
    """
    (tmp_path / "genau").mkdir()
    (tmp_path / "genau" / "__init__.py").write_text("", encoding="utf-8")
    (tmp_path / "genau" / "window.py").write_text(
...
def test_the_suite_imports_its_own_tree_from_a_foreign_directory(tmp_path: Path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pytest",
         str(PROJECT_ROOT / "tests" / "test_genau_window.py"),
...
def test_a_tree_already_imported_before_the_pin_is_named_outright(tmp_path: Path):
    ...
    Reached by a plugin that imports `genau` before conftest does — `sys.modules`
    then holds the decoy's, whatever the path is rearranged to afterwards.
    """
    decoy = _decoy_tree(tmp_path)
    (decoy / "early.py").write_text("import genau\n", encoding="utf-8")
...
def test_a_module_this_tree_does_not_have_is_not_served_from_another_checkout(
    ...
    (elsewhere / "genau").mkdir(parents=True)
    (elsewhere / "genau" / "__init__.py").write_text("", encoding="utf-8")
    (elsewhere / "genau" / "gone_from_here.py").write_text("", encoding="utf-8")
...
def _shadow_probe(elsewhere: Path) -> str:
    ...
    return (
        ...
        f"import shadow; shadow.MAPPING['genau'] = {str(elsewhere / 'genau')!r}; "
        "sys.meta_path.append(shadow._EditableFinder); "
        "import importlib.util; "
        "print('shadowed' if importlib.util.find_spec('genau.gone_from_here') "
        "else 'the stand-in did not shadow -- this test proves nothing'); "
        f"sys.path.insert(0, {str(PROJECT_ROOT / 'tests')!r}); "
        "import conftest; "
        "print('resolved' if importlib.util.find_spec('genau.gone_from_here') "
        "else 'refused')"
    )
